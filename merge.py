...
if mode == '0':
    ...
    #example
    with h5py.File(outDir+'example.h5','w') as opt:
        for i in range(900):
            if i%100==0:
                print(i)
            ipt = h5py.File(inDir+str(i+1000)+'.h5')
            opt.create_dataset(str(i).zfill(4)+'/isoE',data=ipt['isoE'],dtype='float32',compression='gzip')
            ipt.close()
    print('example data generated')
elif mode == '2':
    # answer
    with h5py.File(outDir+'answer.h5','w') as opt:
        for i in range(900):
            if i%100==0:
                print(i)
            ipt = h5py.File(inDir+str(i+100).zfill(4)+'.h5')
